...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { CONTEXT_DEV_API_KEY: requireValue(readEnv('CONTEXT_DEV_API_KEY') ?? client.apiKey, 'set CONTEXT_DEV_API_KEY environment variable or provide apiKey client option'), CONTEXT_DEV_BASE_URL: readEnv('CONTEXT_DEV_BASE_URL') ?? client.baseURL ?? undefined };
