...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; baseline_created: boolean; change_detected: boolean; change_detection_type: 'exact' | 'semantic'; credits_charged: number; monitor_id: string; run_type: 'baseline' | 'scheduled'; status: 'queued' | 'running' | 'completed' | 'failed' | 'skipped'; target_type: 'page' | 'sitemap' | 'extract'; change_id?: string; completed_at?: string; error?: { code: string; message: string; }; skip_reason?: 'insufficient_credits' | 'monitor_paused' | 'superseded'; started_at?: string; webhook_deliveries?: { attempted_at: string; error: object; event: 'change.detected' | 'run.completed'; event_id: string; http_status: number; status: 'delivered' | 'rejected' | 'failed' | 'skipped_unsafe_url'; }[]; webhook_delivery?: { attempted_at: string; error: object; event: 'change.detected' | 'run.completed'; event_id: string; http_status: number; status: 'delivered' | 'rejected' | 'failed' | 'skipped_unsafe_url'; }; }[]; has_more: boolean; next_cursor: string; }",
    markdown:
