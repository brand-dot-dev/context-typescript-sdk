...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['domain: string;', 'numCompetitors?: number;', 'tags?: string[];', 'timeoutMS?: number;'],
    response:
  ...
  },
  {
    ...
    params: [
      "{ domain: string; type: 'by_domain'; force_language?: string; maxAgeMs?: number; maxSpeed?: boolean; tags?: string[]; timeoutMS?: number; } | { name: string; type: 'by_name'; country_gl?: string; force_language?: string; maxAgeMs?: number; maxSpeed?: boolean; tags?: string[]; timeoutMS?: number; } | { email: string; type: 'by_email'; force_language?: string; maxAgeMs?: number; maxSpeed?: boolean; tags?: string[]; timeoutMS?: number; } | { ticker: string; type: 'by_ticker'; force_language?: string; maxAgeMs?: number; maxSpeed?: boolean; tags?: string[]; ticker_exchange?: string; timeoutMS?: number; } | { direct_url: string; type: 'by_direct_url'; tags?: string[]; timeoutMS?: number; } | { transaction_info: string; type: 'by_transaction'; city?: string; country_gl?: string; force_language?: string; high_confidence_only?: boolean; maxSpeed?: boolean; mcc?: string | number; phone?: string | number; tags?: string[]; timeoutMS?: number; };",
    ],
    response:
  ...
  },
  {
    ...
    response:
      "{ id: string; change_detection: object | object; created_at: string; initial_run_id: string; mode: 'web'; name: string; schedule: object; status: 'active' | 'paused' | 'failed'; target: object | object | object; updated_at: string; baseline?: object | object | object; last_change_at?: string; last_error?: object; last_run_at?: string; next_run_at?: string; tags?: string[]; webhook?: object; webhook_failure?: object; }",
    markdown:
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; change_detection: object | object; created_at: string; mode: 'web'; name: string; schedule: object; status: 'active' | 'paused' | 'failed'; target: object | object | object; updated_at: string; baseline?: object | object | object; last_change_at?: string; last_error?: object; last_run_at?: string; next_run_at?: string; tags?: string[]; webhook?: object; webhook_failure?: object; }[]; has_more: boolean; next_cursor: string; }",
    markdown:
  ...
  },
  {
    ...
    response:
      "{ id: string; change_detection: object | object; created_at: string; mode: 'web'; name: string; schedule: object; status: 'active' | 'paused' | 'failed'; target: object | object | object; updated_at: string; baseline?: object | object | object; last_change_at?: string; last_error?: object; last_run_at?: string; next_run_at?: string; tags?: string[]; webhook?: object; webhook_failure?: object; }",
    markdown:
  ...
  },
  {
    ...
    params: ['monitor_id: string;'],
    response: '{ monitor_id: string; queued: boolean; run_id: string; }',
    markdown:
      "## run\n\n`client.monitors.run(monitor_id: string): { monitor_id: string; queued: boolean; run_id: string; }`\n\n**post** `/monitors/{monitor_id}/run`\n\nTriggers an immediate run of the monitor outside its normal schedule. The run is queued and processed asynchronously.\n\n### Parameters\n\n- `monitor_id: string`\n\n### Returns\n\n- `{ monitor_id: string; queued: boolean; run_id: string; }`\n\n  - `monitor_id: string`\n  - `queued: boolean`\n  - `run_id: string`\n\n### Example\n\n```typescript\nimport ContextDev from 'context.dev';\n\nconst client = new ContextDev();\n\nconst response = await client.monitors.run('mon_123');\n\nconsole.log(response);\n```",
    perLanguage: {
      php: {
        method: 'monitors->run',
        example:
  ...
  },
  {
    ...
    params: ['batch_id: string;', 'cursor?: string;', 'limit?: number;'],
    response:
  ...
  },
  {
    ...
    response:
      "{ match: { person: { current_role_status: 'present' | 'none' | 'unknown'; education: object[]; experience: object[]; skills: string[]; social_urls: string[]; website_urls: string[]; avatar_url?: string; bio?: string; checked_at?: string; current_role?: object; email?: string; last_updated?: string; location?: object; name?: object; }; score: number; status: 'candidate'; } | { person: null; score: null; status: 'not_found'; }; key_metadata?: { credits_consumed: number; credits_remaining: number; }; }",
    markdown:
      "## enrich\n\n`client.people.enrich(company?: { domain?: string; name?: string; }, education?: { degree?: string; field_of_study?: string; graduation_year?: number; institution?: { domain?: string; name?: string; }; }[], email?: string, location?: { city?: string; country?: string; region?: string; }, name?: { first?: string; last?: string; }, social_urls?: string[], tags?: string[], timeoutMS?: number): { match: object | object; key_metadata?: object; }`\n\n**post** `/people/enrich`\n\nFinds and normalizes the best available person candidate from additive identity clues, then assigns an identity match score from 0 to 100. Available on all paid plans. Successful requests cost 20 credits. Disposable and free email addresses (like gmail.com, yahoo.com) will throw a 422 error.\n\n### Parameters\n\n- `company?: { domain?: string; name?: string; }`\n  - `domain?: string`\n  - `name?: string`\n\n- `education?: { degree?: string; field_of_study?: string; graduation_year?: number; institution?: { domain?: string; name?: string; }; }[]`\n\n- `email?: string`\n\n- `location?: { city?: string; country?: string; region?: string; }`\n  - `city?: string`\n  - `country?: string`\n  - `region?: string`\n\n- `name?: { first?: string; last?: string; }`\n  - `first?: string`\n  - `last?: string`\n\n- `social_urls?: string[]`\n\n- `tags?: string[]`\n  Optional tags for tracking usage. Up to 20 tags, each 1 to 50 characters.\n\n- `timeoutMS?: number`\n  Optional timeout in milliseconds for the request. If the request takes longer than this value, it will be aborted with a 408 status code. Maximum allowed value is 300000ms (5 minutes).\n\n### Returns\n\n- `{ match: { person: { current_role_status: 'present' | 'none' | 'unknown'; education: object[]; experience: object[]; skills: string[]; social_urls: string[]; website_urls: string[]; avatar_url?: string; bio?: string; checked_at?: string; current_role?: object; email?: string; last_updated?: string; location?: object; name?: object; }; score: number; status: 'candidate'; } | { person: null; score: null; status: 'not_found'; }; key_metadata?: { credits_consumed: number; credits_remaining: number; }; }`\n\n  - `match: { person: { current_role_status: 'present' | 'none' | 'unknown'; education: { institution: { name: string; domain?: string; }; degree?: string; description?: string; end_date?: { year: number; day?: number; month?: number; }; field_of_study?: string; start_date?: { year: number; day?: number; month?: number; }; }[]; experience: { organization: { name: string; domain?: string; }; title: string; description?: string; end_date?: { year: number; day?: number; month?: number; }; is_current?: boolean; location?: string; start_date?: { year: number; day?: number; month?: number; }; }[]; skills: string[]; social_urls: string[]; website_urls: string[]; avatar_url?: string; bio?: string; checked_at?: string; current_role?: { organization: { name: string; domain?: string; }; title: string; description?: string; end_date?: { year: number; day?: number; month?: number; }; is_current?: boolean; location?: string; start_date?: { year: number; day?: number; month?: number; }; }; email?: string; last_updated?: string; location?: { city?: string; country?: string; country_code?: string; display?: string; region?: string; }; name?: { first?: string; full?: string; last?: string; }; }; score: number; status: 'candidate'; } | { person: null; score: null; status: 'not_found'; }`\n  - `key_metadata?: { credits_consumed: number; credits_remaining: number; }`\n\n### Example\n\n```typescript\nimport ContextDev from 'context.dev';\n\nconst client = new ContextDev();\n\nconst response = await client.people.enrich();\n\nconsole.log(response);\n```",
    perLanguage: {
      php: {
